...
namespace beauty {
    ...
    void session<udp>::receive(endpoint<udp> ep, bool async)
    {
        if (!_socket.is_open()) {
            error_code ec;
        ...
        if (async) {
            _socket.async_receive_from(mbuf, ep,
                asio::bind_executor(
                    _strand, [me = this->shared_from_this(), ep](auto ec, auto tbytes) {
    ...
    void session<tcp>::do_read(bool async)
    {
